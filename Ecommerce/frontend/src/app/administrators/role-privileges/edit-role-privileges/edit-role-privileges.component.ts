...
@Component({
  ...
  styleUrls: ['./edit-role-privileges.component.scss']
})
export class EditRolePrivilegesComponent implements OnInit {

  @Output() afterEditRolePrivileges = new EventEmitter<string>();
  ...
  ngOnInit(): void {
    ...
    this.searchRoleMenu();
    this.searchRoles();
    this.rolePrivileges = window.localStorage.getItem("rolePrivilegesId");
  ...
  initForm() {
    this.editRoleMenuForm = this.formBuilder.group(
      {
        rolesPrivilegesId: ['', Validators.required],
  ...
  async getRoleMenuDetail(){
    if(this.rolePrivileges !== ''){
      ...
      Swal.showLoading();
      await this.service
        .doSearchDetail( parseInt(this.rolePrivileges), params )
        .then((data) => {
          Swal.close();
          let response = data;
          if (response.code === 0) {
  ...
  editRoleMenu(){
    ...
    this.editRoleMenuForm.patchValue({id: this.rolePrivileges});
      ...
      .then((data) => {
        ...
        if (response.code === 0) {
          Swal.fire({
            ...
            Swal.close();
            this.afterEditRolePrivileges.emit('completed');
          });
        } else {
          Swal.fire({
            icon: "error",
